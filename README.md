...
The app is deployed on Railway at `https:/server-secret-echo-production.up.railway.app/`. Note that WebSocket connections on Railway’s default `*.up.railway.app` domains are terminated after 30 seconds due to proxy limitations. To ensure persistent WebSocket connections, set up a custom domain

### Deploying to Railway

1. Push your code to a GitHub repository.
2. Create a new project on Railway and link it to your GitHub repository.
3. Set the environment variables in the Railway dashboard (e.g., `MONGODB_URI`, `TOKEN_SECRET`, `GEMINI_AI_API_KEY` and other `.env` variables).
4. Railway will automatically build and deploy your app using the `Dockerfile`.
